...
Research now uses `openai.gpt-5.6-luna` through Amazon Bedrock Mantle, signed with refreshable IAM credentials. No separate OpenAI API key is needed. Model agreement activation was submitted; live inference remains gated until access and smoke checks pass. The empty OpenAI secret was removed with its seven-day recovery period.
...
Local verification: 62 tests, type checking and production builds passed. Initial live checks passed for unauthenticated API rejection and the worker readiness guard. Rejected model checks did not run inference; their budget reservations were released. Hosted build, authenticated acceptance, full live research and the seven-day quality/cost pilot remain to be recorded.
